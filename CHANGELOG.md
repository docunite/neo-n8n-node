...
### Fixed
- **Document Blob Download**: Dateinamen werden jetzt aus dem `Content-Disposition` Header der API-Response extrahiert
  - Betrifft beide Operationen: `Get Document PDF Blob` und `Get Document Original Blob`
  - Original-Dateiname vom Server wird verwendet anstatt generierter Namen
  - Einfache String-Extraktion ohne zusätzliche Dependencies
  - Fallback auf generierte Namen, falls Header nicht vorhanden ist
